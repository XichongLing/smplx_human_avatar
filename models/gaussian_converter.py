...
class GaussianConverter(nn.Module):
    ...
    def set_optimizer(self):
        opt_params = [
            ...
            #  {'params': [p for n, p in self.deformer.garm_simulator.named_parameters() if 'latent' not in n],
            #  'lr': self.cfg.opt.get('garm_simulator_lr', 0.)},
            # {'params': [p for n, p in self.deformer.garm_simulator.named_parameters() if 'latent' in n],
            #  'lr': self.cfg.opt.get('garm_simulator_lr', 0.), 'weight_decay': self.cfg.opt.get('latent_weight_decay', 0.05)},
