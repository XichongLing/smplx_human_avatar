...
from models.network_utils import get_skinning_mlp, get_ImplicitNet, get_deformation_mlp, get_bone_encoder, HierarchicalPoseEncoder, VanillaCondMLP
...
class DeformationGraph(Garm_Simulator):
    def __init__(self, cfg, metadata, garm_simulator_args):
        ...
        self.condition = "pose+frame"
        if self.condition == "pose+frame":
        ...
        elif self.condition == "bone_trans+posenc+hashgrid":
    ...
    def forward(self, gaussians, iteration, camera, time_enc, **kwargs):
        ...
        nodes_deformed = self.forward_graph(self.vb_model.get_virtual_joints(), vb_rotation, vb_translation, camera)
    ...
    def forward_graph(self, nodes, vb_rotation, vb_translation, camera):
        vb_rotation_calib = torch.matmul(vb_rotation, camera.root_orient_mat.cuda())
        vb_translation_calib = vb_translation + torch.tensor(camera.transl).cuda()
        transform_mat = to_transform_mat(vb_rotation_calib,vb_translation_calib.unsqueeze(2))
...
def save_ply_garmweights(garm_xyz, deformed_xyz, virtual_joints, pred_weights, iteration, save_dir):
    ...
    file_garm = save_dir + "/weighted_cano_garm_{0}.ply".format(iteration)
    file_deformed = save_dir + "/weighted_deformed_garm_{0}.ply".format(iteration)    
    file_vjoints = save_dir + "/weighted_virtual_joints_{0}.ply".format(iteration)
