...
class FDressMultiseqDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        # Take1
        if split == 'train':
            self.subject = cfg.get('train_subject')
        else:
            # val test predict all group to test
            split == "test"
            self.subject = cfg.get('test_subject')
        self.split = split
        ...
        assert len(frames) == len(self.subject)
        total_frames = 0
        frame_indices = []
        for frame in frames:
            total_frames += len(range(frame[0], frame[1], frame[2]))
            frame_indices.append(list(range(frame[0], frame[1], frame[2])))
        print("the number of total frames is: ", total_frames)
        # need camera here
        ...
        self.cameras = [{'K': camera['intrinsics'],
            'R': camera['extrinsics'][:3, :3],
            'T': camera['extrinsics'][:3, 3]}] * total_frames
        ...
        start_frame = [frame[0] for frame in frames]
        end_frame = [frame[1] for frame in frames]
        sampling_rate = [frame[2] for frame in frames]
        ...
        if split == 'predict':
            ...
            if self.model_type == 'smpl':
                model_files = []
                for seq in self.subject:
                    model_file = sorted(
                    glob.glob(os.path.join(self.root_dir, self.style, seq, 'SMPL_processed/*.npz')))
                    model_files = model_files + model_file
            elif self.model_type == 'smplx':
                model_files = []
                for seq in self.subject:
                    model_file  = sorted(
            ...
            # manuallly reset the start_frame, end_frame, sampling_rate
            frame_slice = slice(0, len(model_files), 1)
            model_files = model_files[frame_slice]
            frames = frames[frame_slice]
        ...
        if split == 'predict' or cfg.freeview:
            ...
            img_files, mask_files, segmentation_files = [], [], []
            for seq in self.subject:
                img_file = sorted(glob.glob(os.path.join(self.root_dir, self.style, seq, "Capture", self.camera_idx,"images/*.png")))[
                    frame_slice]
                img_files = img_files + img_file
                mask_file = \
                sorted(glob.glob(os.path.join(self.root_dir, self.style, seq, "Capture", self.camera_idx,"masks/*.png")))[frame_slice]
                mask_files = mask_files + mask_file
                segmentation_file = sorted(glob.glob(os.path.join(self.root_dir, self.style, seq, "Capture", self.camera_idx,"labels/label*.png")))[frame_slice]
                segmentation_files = segmentation_files + segmentation_file
            for d_idx, f_idx in enumerate(frames):
