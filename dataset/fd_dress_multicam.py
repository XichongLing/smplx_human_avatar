...
class FDressMulticamDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        self.camera_idx_train = cfg.camera_idx_train
        self.camera_idx_val = cfg.camera_idx_val
        # Take1
        if split == 'train':
        ...
        if self.split == 'train':
            cameras=[]
            for cam in self.camera_idx_train:
                camera = [{'K': cam_data[cam]['intrinsics'],
                            'R': cam_data[cam]['extrinsics'][:3, :3],
                            'T': cam_data[cam]['extrinsics'][:3, 3]}] * len(range(frames[0], frames[1], frames[2]))
                cameras = cameras + camera
            self.cameras = cameras

        else:
            # for test, val, predict, only one camera is used
            camera = cam_data[self.camera_idx_val]
        ...
        if split == 'predict':
            ...
            if self.split == 'train':
                ...
                frame_slice = slice(start_frame, end_frame, sampling_rate)
                model_files = model_files[frame_slice]
                model_files = model_files * len(self.camera_idx_train)
                frames = frames[frame_slice]
            else:
                if self.model_type == 'smpl':
                    model_files = sorted(
        ...
        if split == 'predict' or cfg.freeview:
            ...
            if self.split == 'train':
                ...
                for cam in self.camera_idx_train:
                    ...
                    segmentation_file = sorted(glob.glob(os.path.join(self.root_dir, self.style, self.subject, "Capture", cam,"labels/label*.png")))[frame_slice]
                    img_files = img_files + img_file    
                    mask_files = mask_files + mask_file
                    segmentation_files = segmentation_files + segmentation_file
                frames = list(range(len(img_files)))
                # frames = 


            else: 
                img_files = sorted(glob.glob(os.path.join(self.root_dir, self.style, self.subject, "Capture", self.camera_idx_val,"images/*.png")))[
                        frame_slice]
                mask_files = \
                        sorted(glob.glob(os.path.join(self.root_dir, self.style, self.subject, "Capture", self.camera_idx_val,"masks/*.png")))[frame_slice]

                segmentation_files = sorted(glob.glob(os.path.join(self.root_dir, self.style, self.subject, "Capture", self.camera_idx_val,"labels/label*.png")))[frame_slice]
            for d_idx, f_idx in enumerate(frames):
