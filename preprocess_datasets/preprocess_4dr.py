...
def preprocess_4dr(root_dir, subj, outfit, seqs, model_type='smpl', gender = 'female'):

    data_dir = os.path.join(root_dir, subj, outfit)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    if seqs == [] or seqs == None:
        seqs = glob.glob(os.path.join(data_dir, '*'))
    else:
        seqs = [os.path.join(data_dir, seqs)]
    if model_type == 'smplx':
...
if __name__ == '__main__':
    ...
    DATA_ROOT = '../data_lx/datasets/4Dress'  
