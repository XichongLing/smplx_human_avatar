...
def render_pixel_label_colors(labels):
    ...
    images = np.ones((*labels.shape[:3], 3)) * 0
...
def subj_outfit_seq_render_pixel_labels(dataset_dir, subj, outfit, seq):
    ...
    for cam_id in camera_list:
        if os.path.exists(os.path.join(capture_dir, cam_id, 'labels')):
            delete_dir(os.path.join(capture_dir, cam_id, 'labels'))
        os.makedirs(os.path.join(capture_dir, cam_id, 'labels'), exist_ok=True)
        # locate camera_list, make labels folders
        camera_list = RELEASE_CAMERAS
        for cam_id in camera_list:
            label_dir_path = os.path.join(capture_dir, cam_id, 'labels')
            if os.path.exists(label_dir_path) and os.path.isdir(label_dir_path):
                # delete files inside the directory
                file_list = os.listdir(label_dir_path)
...
if __name__ == "__main__":
    ...
    # ### render all takes under the given subject and outfit
    # seq_dirs = glob.glob(os.path.join(DATASET_DIR, args.subj,args.outfit, '*'))
    # seq_list = [os.path.basename(seq_dir) for seq_dir in seq_dirs]
    # for seq in seq_list:
    #     subj_outfit_seq_render_pixel_labels(dataset_dir=DATASET_DIR, subj=args.subj, outfit=args.outfit, seq=seq)
    ...
    # subject_list = ['00123', '00185','00187']
    # # subject_list = ['00185','00187']
    # outfit_list = ['Outer', 'Inner']
    # seq_list = ['Take1', 'Take2', 'Take3', 'Take4', 'Take5', 'Take6', 'Take7', 'Take8', 'Take9', 'Take10', 'Take11', 'Take12', 'Take13']
    # # for seq_idx in range(9, 14):
    # #     subj_outfit_seq_render_pixel_labels(dataset_dir=DATASET_DIR, subj=subject_list[0], outfit=outfit_list[0], seq=seq_list[seq_idx - 1])

    # for subj_idx in range(1,3):
    #     for seq_idx in range(1,10):
    #         subj_outfit_seq_render_pixel_labels(dataset_dir=DATASET_DIR, subj=subject_list[subj_idx], outfit=outfit_list[1], seq=seq_list[seq_idx - 1])